...
\ForEach{$n \in \{  2..100, \ldots, `r format_int_latex(max(settings$scale_efficiency$ns))` \}$}{
  ...
  \For{$i \gets 1..\textit{repetitions}$}{
    ...
    $y_{\SQAD,i} \gets \SQAD_n(x)$\\
    $y_{\OQAD,i} \gets \OQAD_n(x)$\\
  }
  $e(\MAD_n) \gets \V_s(y_{\SD,\{i\}}) \;/\; \V_s(y_{\MAD,\{i\}})$\\
  $e(\SQAD_n) \gets \V_s(y_{\SD,\{i\}}) \;/\; \V_s(y_{\SQAD,\{i\}})$\\
  $e(\OQAD_n) \gets \V_s(y_{\SD,\{i\}}) \;/\; \V_s(y_{\OQAD,\{i\}})$\\
}
\end{algorithm}
...
\ForEach{$n \in \{  2..100, \ldots, `r format_int_latex(max(settings$location_efficiency$ns))` \}$}{
  ...
  \For{$i \gets 1..\textit{repetitions}$}{
    $x \gets \textrm{GenerateRandomSample}(\textrm{Distribution} = \mathcal{N}(0, 1),\, \textrm{SampleSize} = n)$\\
    $y_{\mean,i} \gets \mean_n(x)$\\
    $y_{\SM,i} \gets \SM_n(x)$\\
    $y_{\STHDME,i} \gets \STHDME_n(x)$\\
    $y_{\OTHDME,i} \gets \OTHDME_n(x)$\\
  }
  $e(\SM_n) \gets \V(y_{\mean,\{i\}}) \;/\; \V(y_{\SM,\{i\}})$\\
  $e(\STHDME_n) \gets \V(y_{\mean,\{i\}}) \;/\; \V(y_{\STHDME,\{i\}})$\\
  $e(\OTHDME_n) \gets \V(y_{\mean,\{i\}}) \;/\; \V(y_{\OTHDME,\{i\}})$\\
}
\end{algorithm}
